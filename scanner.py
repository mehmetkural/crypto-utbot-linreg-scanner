...
def get_usdt_symbols(min_volume=MIN_24H_QUOTE_VOLUME_USDT):
    ...
    info = requests.get(f"{BINANCE_BASE}/api/v3/exchangeInfo", timeout=REQUEST_TIMEOUT).json()
    tradable = set()
    ...
    tickers = requests.get(f"{BINANCE_BASE}/api/v3/ticker/24hr", timeout=REQUEST_TIMEOUT).json()
    qualified = []
    for t in tickers:
        sym = t["symbol"]
        if sym in tradable:
